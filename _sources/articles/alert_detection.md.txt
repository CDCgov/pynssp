...
>>> myProfile = pynssp.create_profile()
...
```python
# Subset the time series with the anomalies for Region 4
...
```python
# Apply the Adaptive Multiple Regression algorithm
...
```python
# Apply the Regression/EWMA Switch algorithm
...
```python
# Load synthesized time series data for scenario1
>>> synth_ts1 = pynssp.get_scenario1()
>>> synth_ts1.head()
```

```python
# Apply Negative Binomial detector
...
>>> sns.lineplot(data=df_nbinom, x='date', y='threshold', color='#A50026', linewidth=0.5, label='Threshold', ax=ax)
>>> sns.scatterplot(data=df_nbinom.loc[df_nbinom['alarm']], x='date', y='cases', color='#A50026', ax=ax)
...
# Set legend
>>> ax.legend(title="Series", loc='lower center', bbox_to_anchor=(0.5, -0.3), ncol=2, framealpha=1, edgecolor='black')
...
>>> sns.lineplot(data=df_serfling, x='date', y='threshold', color='#A50026', linewidth=0.5, label='Threshold', ax=ax)
>>> sns.scatterplot(data=df_serfling.loc[df_nbinom['alarm']], x='date', y='cases', color='#A50026', ax=ax)
